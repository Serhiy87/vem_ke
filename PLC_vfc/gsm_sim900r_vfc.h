...
inline static void GSM_Auto(){
	...
	switch(GSM_State){
		...
		case GSM_PowerOn:
			...
			StartTimer32(TD_GSM_Reset, 61UL*6000);	// Перезапуск модема каждые 61 мин
			GSM_PWRCNTRL_ON();
			InitFIFO();
...
//			InitFIFO();
		...
		case GSM_SEND_CSQ:
			...
			StartTimer16(TD_RSSI, 1500);
			GSM_State = GSM_ServerIdle;
			break;
		case GSM_WAIT_CSQ:
				GSM_RSSI = 255; GSM_BER = 255;
				sscanf(GSM_RxStr,"+CSQ: %hhu,%hhu", (unsigned char*)&GSM_RSSI, (unsigned char*)&GSM_BER);
				GSM_State = GSM_ServerIdle;
			break;
		//------------------------
