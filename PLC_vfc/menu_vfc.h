...
uint8_t CurrLine, CurrField=NullPos, CurrPos=NullPos, CurrLCD;
...
IsBlink(void)
{
	return Blink>=8 && Blink<=10 || Blink>=14;
}
...
{
	if(IsBlink())
		*(Param->Pos+Param->Size-CurrPos-(Param->Prec&&CurrPos>=Param->Prec?2:1)) = Cursor;
}
