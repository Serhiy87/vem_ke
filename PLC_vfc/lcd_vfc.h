/*
	02.04.2012 * Kononenko *  Revised for vmd2_1 - LCDBackLight

	01.08.2012 Kononenko
	Добавлены символы 'є','Є','ї','Ї'. Апостроф: Alt+39. Корректировка Win2LCD[]

	27.09.2013 Kononenko для vfc. 

	12.12.2013 - Kononenko -  Add LCD Full(!) reset one time per 20 frames

	14.12.2013 - Kononenko -  Use IntOff, IntOn in LCD_Out()!

	27.12.2013 - Kononenko - Revised for vfc2

	08.02.2014 - Kononenko - При сильных помехах не всегда помогает - изображение сдвигается
				Восстанавливает 100% - команда RETURN HOME
				Но при этом на некоторых моделях ЖКИ изображение заметно дёргается при переинициализации
				Дёргается на Fordata. Не дёргается на Winstar, Bolymin.
				Вводится #define NOT_UDE_LCD_RETURN_HOME_CMD
*/


#if TypeLCD==216
	#define LCDXSz 16
	#define LCDYSz 2
#elif TypeLCD==420
	#define LCDXSz 20
	#define LCDYSz 4
#endif

//Буфер на один ряд по X больше для символа завершения строки
//Буфер на один ряд по Y больше для защиты от несакционированого вмешательств
char LCD_Buf[LCDYSz+1][LCDXSz+1];

#define CharUp			0xA7
#define CharDown		0xB7
// Win '|' <=> LCD Degree
#define CharCursor	0x10
#define CharHand		0x11

volatile static uint8_t LCD_Rst_Flg;

static prog_uint8_t Win2LCD[] = {
	
};
static prog_uint8_t CGRAM_LCD[8][8] = {
	{	// Cursor
		0b00000,
		0b00100,
		0b00110,
		0b11111,
		0b00110,
		0b00100,
		0b00000,
		0b00000
	},
	{	// Degree
		0b00110,
		0b01001,
		0b01001,
		0b00110,
		0b00000,
		0b00000,
		0b00000,
		0b00000
	},
	{	// Hand
		0b00000,
		0b01000,
		0b01000,
		0b01110,
		0b01111,
		0b11111,
		0b01111,
		0b00110
	},
	{	// 'э'
		0b00000,
		0b00000,
		0b01110,
		0b10001,
		0b11100,
		0b10001,
		0b01110,
		0b00000
	},
	{	// 'Є'
		0b01110,
		0b10001,
		0b10000,
		0b11100,
		0b10000,
		0b10001,
		0b01110,
		0b00000
	},
	{	// 'ї'
		0b01010,
		0b00000,
		0b01100,
		0b00100,
		0b00100,
		0b00100,
		0b01110,
		0b00000
	},
	{	// 'Ї'
		0b01010,
		0b01110,
		0b00100,
		0b00100,
		0b00100,
		0b00100,
		0b01110,
		0b00000
	}
};

#define LCD_Delay 50	// 40 us needs

static uint8_t TD_LCD, TD_LCD_Rst;

void LCDBackLightInit(void);
void LCDBackLightOn(void);
void LCDBackLightOff(void);
void LCDBackLightStartTimer(void);

// ~~~~~~~~~~~~~~~~~~
static void
SendTetr(uint8_t Val)
{
	IntOff();

	if (Val &(1<<0)) Port_LCD_DB4 |=DB4; else Port_LCD_DB4 &=~DB4;
	if (Val &(1<<1)) Port_LCD_DB5 |=DB5; else Port_LCD_DB5 &=~DB5;
	if (Val &(1<<2)) Port_LCD_DB6 |=DB6; else Port_LCD_DB6 &=~DB6;
	if (Val &(1<<3)) Port_LCD_DB7 |=DB7; else Port_LCD_DB7 &=~DB7;
	
	Port_LCD_E |=LCD_E;
	_delay_us(1);		// 450 ns needs
	Port_LCD_E &=~LCD_E;
	_delay_us(1);		// 10 ns needs
	
	IntOn();
}

// ~~~~~~~~~~~~~~
static void
Send(uint8_t Val)
{
	SendTetr(Val>>4);
	SendTetr(Val&0xF);
}

// ~~~~~~~~~~~~~~~~
static void
Send_I(uint8_t Val)
{
	Send(Val);
	Delay_us(LCD_Delay);
}

// ~~~~~~~~~~~
static void
LCD_FuncSet0(void)
{
	IntOff();

	Port_LCD_DB4  |=DB4;
	Port_LCD_DB5  |=DB5;
	Port_LCD_DB6 &=~DB6;
	Port_LCD_DB7 &=~DB7;

	IntOn();
}
// ~~~~~~~~~~~
static void
LCD_FuncSet1(void)
{
	IntOff();

	Port_LCD_DB4 &=~DB4;
	Port_LCD_DB5  |=DB5;
	Port_LCD_DB6 &=~DB6;
	Port_LCD_DB7 &=~DB7;
	
	IntOn();
}
// ~~~~~~~~~~~
static void
LCD_Rst0(void)
{
	IntOff();
	Port_LCD_RS &=~LCD_RS;
	IntOn();

	LCD_FuncSet0();
}

// ~~~~~~~~~~~
static void
LCD_Rst1(void)
{
	LCD_FuncSet0();
	Delay_us(200);				// 100 us needs
	LCD_FuncSet0();
	Delay_us(LCD_Delay);
	LCD_FuncSet1();
	Delay_us(LCD_Delay);
	
	IntOff();
	Port_LCD_RS |=LCD_RS;
	IntOn();
}

// ~~~~~~~~~~~~
void
LCD_Init (void)
{
	uint8_t i, j;

	// LCD pins
	#ifdef Port_LCD_RW
		Port_LCD_RW &= ~LCD_RW;
		DDR_LCD_RW |= LCD_RW;
	#endif

	Port_LCD_RS &= ~LCD_RS;	
	DDR_LCD_RS |=LCD_RS;

	Port_LCD_E &= ~LCD_E;	
	DDR_LCD_E |=LCD_E;

	DDR_LCD_DB4 |=DB4;
	DDR_LCD_DB5 |=DB5;
	DDR_LCD_DB6 |=DB6;
	DDR_LCD_DB7 |=DB7;

	// LCD initialization
	Delay_ms(50);		// 40 ms needs to 2.7 V

	LCD_Rst0();
	Delay_ms(5);		// 4.1 ms needs
	LCD_Rst1();

	Port_LCD_RS &=~LCD_RS;

	Send_I(0b00101000);
	Send_I(0b00001100);
	Send_I(0b00000110);

	Send_I(0b01000000);
	Port_LCD_RS |=LCD_RS;
	for (i=0; i<8; i++)
		for (j=0; j<8; j++)
			Send_I(prb(CGRAM_LCD[i]+j));

	Port_LCD_RS &=~LCD_RS;
	Send(0b00000001);
	Delay_ms(5);				// By trial
	Port_LCD_RS |=LCD_RS;

	TD_LCD 		= Timer8SysAlloc(1);	// Frame Timer
	TD_LCD_Rst 	= Timer8SysAlloc(1);	// LCD_RST Timer

	for (i=0; i<LCDYSz; i++)
		for (j=0; j<LCDXSz; j++)
			LCD_Buf[i][j] = ' ';

	#ifdef RTYP
		*(uint8_t*)RTYP = R_PLC_Type;
	#endif

	LCDBackLightInit();
}

// ~~~~~~~~~~
uint8_t
LCD_Rdy(void)
{
	uint8_t Rdy = Timer8Stopp(TD_LCD);
	if (Rdy) StartTimer8(TD_LCD, 6);
	return Rdy;
}

// ~~~~~~~~~~ 
// Выодит на экран только один символ, одна команда
// Execution time 
void LCD_Out(void)
{

	static uint8_t N;

	if( (N>=20) && !LCD_Rst_Flg ){
		N = 0;
		LCD_Rst0();
		LCD_Rst_Flg = 1;
		StartTimer8(TD_LCD_Rst,2);// 5ms need
		return;
	}

	if (LCD_Rst_Flg) {
		if(Timer8Stopp(TD_LCD_Rst)){
			switch(LCD_Rst_Flg){
				case 1:
					LCD_FuncSet0();
					Delay_us(150);
					break;
				case 2:
					LCD_FuncSet0();
					break;
				case 3:
					LCD_FuncSet1();
					break;
				case 4:
					Send(0b00101000);
					break;
				case 5:
					Send(0b00001100);
					break;
				case 6:
					Send(0b00000110);
					break;
				case 7:
					Send(0b01000000);
					IntOff();
					Port_LCD_RS |=LCD_RS;
					IntOn();
					break;
				case 8:{
						static uint8_t i,j;
						LCD_Rst_Flg = 7;
						Send(prb(CGRAM_LCD[i]+j));
						if(j<7) j++;
						else{
							j=0;
							if(i<7)i++;
							else{i=0;LCD_Rst_Flg = 8;}
						}
					}
					break;
				case 9:
					IntOff();
					Port_LCD_RS &=~LCD_RS;
					IntOn();
					#ifdef NOT_USE_LCD_RETURN_HOME_CMD
						Send(128);			// Установить курсор в нулевую позицию
					#else
						Send(0b00000010);	//RETURN HOME
						StartTimer8(TD_LCD_Rst,2);// 5ms need
					#endif
					IntOff();
					Port_LCD_RS |=LCD_RS;
					IntOn();
				default:
					LCD_Rst_Flg = 255;
			}
			LCD_Rst_Flg++;
		}
	}
	else {
		static uint8_t X, Y;
		static prog_uint8_t LineAddr[] = {128, 192, 148, 212};

		if (X<LCDXSz) {
			Send(
			#ifdef RMON
				*((uint8_t*)RMON+Y*LCDXSz+X) =
			#endif
				prb(Win2LCD+(unsigned char)LCD_Buf[Y][X]));
				X++;
		}
		else {
			X = 0;
			if (++Y>=LCDYSz) {
				Y = 0;
				N++;
			}
			IntOff();
			Port_LCD_RS &=~LCD_RS;
			IntOn();
			
			Send(prb(LineAddr+Y));
			
			IntOff();
			Port_LCD_RS |=LCD_RS;
			IntOn();
		}
	}
	
	Delay_us(50);
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~
static uint8_t TD_BackLight;
uint8_t BackLightMode;
enum BACKLIGHT_TIMES{ BCKLT_10sec, BCKLT_30sec, BCKLT_1min,  BCKLT_3min,  BCKLT_10min};
static uint8_t EEMEM BackLightTimeEE = BCKLT_10sec;

void LCDBackLightInit(void){
	IntOff();	
	DDR_BACKLIGHT |= BACKLIGHT;
	IntOn();	
	TD_BackLight = Timer16SysAlloc(1);
	if(BackLightMode != 2){
		LCDBackLightOn();
		LCDBackLightStartTimer();
	}
}
void LCDBackLightOn(void){
	IntOff();	
	#ifdef BACKLIGHT_INVERSION
		PORT_BACKLIGHT &= ~BACKLIGHT;
	#else
		PORT_BACKLIGHT |= BACKLIGHT;
	#endif
	IntOn();	
}
void LCDBackLightOff(void){
	IntOff();	
	#ifdef BACKLIGHT_INVERSION
		PORT_BACKLIGHT |= BACKLIGHT;
	#else
		PORT_BACKLIGHT &= ~BACKLIGHT;
	#endif
	IntOn();
}
void LCDBackLightStartTimer(void){
	switch(erb(&BackLightTimeEE)){
		case BCKLT_10sec:
			StartTimer16(TD_BackLight, 1000);
			break;
		case BCKLT_30sec:
			StartTimer16(TD_BackLight, 3000);
			break;
		case BCKLT_1min:
			StartTimer16(TD_BackLight, 6000);
			break;
		case BCKLT_3min:
			StartTimer16(TD_BackLight, 18000);
			break;	
		case BCKLT_10min:
			StartTimer16(TD_BackLight, 60000);
			break;
		default:
			StartTimer16(TD_BackLight, 3000);
			break;			
	}
}
void LCDBackLightControl(void){
	switch(BackLightMode){
	case 1: if(Timer16Stopp(TD_BackLight)) LCDBackLightOff();
		break;
	case 2:
		break;
	default: LCDBackLightOn();
	}
}
//~~~~~~~~~~~~~~~~~~~~~~~~~~~
