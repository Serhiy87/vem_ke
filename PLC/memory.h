/*
	...
	typedef PGM_P PROGMEM const prog_str; -> #ifdef _GCC_WRAP_STDINT_H (define with stdint.h AtmelStudio7; define what is not in AVRStudio4)
							typedef uint8_t PROGMEM const prog_uint8_t;
							typedef uint16_t PROGMEM const prog_uint16_t;
							typedef uint32_t PROGMEM const prog_uint32_t;
							typedef char PROGMEM const prog_char;
						  #endif
 						  typedef PGM_P PROGMEM const prog_str;
	_GCC_WRAP_STDINT_H - это дефайн, котрый объ€влен в AtmelStudio7, но нет в AVRStudio4, может быть и какой-то другой. ¬ библиотеках AVRStudio4 строки typedef... 
	уже объ€влены, поэтому в AVRStudio4 этих строк не должно быть, иначе ошибки
