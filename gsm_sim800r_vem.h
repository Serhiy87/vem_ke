/* ~~~~~~~~~~~~~
// 	Modem: SIM900R, 9600 bps,N,1. 	Для EMReader.
	...
	07.04.2015 - Может зависать на этапе получения IP GSM_WAIT_CIFSR_OK. Добавлено if(Timer16Stopp(TD_GSM)) GSM_State = GSM_ReStart1;
				- Полный сброс модема 61 мин TD_GSM_Reset. (бывали случаи когда модем пинговался, все команды нормально 
...
***********	http
	...
	25,09,2015 - Определение протокола по заголовку (анализ начальных байт) uint8_t AppProtocol в режиме GSM_ProtocolMode
				GSM_MultiCon
				Читаем часть ICCID для идентификации. Используем sscanf_P(Buf*, PSTR(...))

	29,09,2015 - Защита от зависания в GSM_DataMode для if(AppProtocol != _HTTP)
	
	30.09.2015 - Портировано на vem

		При регистрации в GPRS, модем переходит в прозраный режим ( =1).
		Периодически модем переконфигурируется для передачи данных и обратно уходит в прозрачный режим.
		Соединение не устанавливается если UART_Soft==1
		В прозрачном режим из очереди FIFO передаётся в UART E_METER(по прерываниям)  и пишется в GSM_RxStr для анализа того чё получили от модема

	01.10.2015 - Замечен случай ICCID2 = 0. Изменнил условия сброса GSM_WAIT_CCID_READ
	...
	23.02.2016 - Добавлен режим CSD и список разрешённых номеров CSD_AllowedNumbers!
...
// ~~~~~~~~~~~
//FIFO
#define GSM_RX_FIFO_SIZE 250		// при условиях до 5мс ScanCycle и GPRS_In_MaxSz =< 50
...
volatile uint8_t GSM_RX_FIFO_End_Transp;		//Индекс последнего прочитаного байта в прозрачном режиме
...
uint8_t Transparent;	// Признак прозрачного режима
...
inline static void GSM_Auto(){
	...
	switch(GSM_State){
		...
		case GSM_PowerOn:
			...
			//StartTimer32(TD_GSM_Reset, 10UL*6000);	// Перезапуск модема каждые 24ч мин
			GSM_PWRCNTRL_ON();
			InitFIFO();
			EM_InitFIFO();
...
/*			GetStringFromFIFO();		// чтобы не было переполнения FIFO
		...
		case GSM_AnalyzeURC:			//Анализ принятой строки
			...
			if( (strstr_P(GSM_RxStr, URC_RING) != NULL) ){
				...
				GSM_State = GSM_CheckNumber;	//проверяем номер и поднимаем трубку
				break;
			}
...
//			InitFIFO();
		...
		/*	if(GSM_CSD == 1){	// В CSD только прозрачный режим
			...
			while(GSM_RX_FIFO_Begin != GSM_RX_FIFO_End){
				...
				static uint8_t GSM_RX_FIFO_End_old;	// Чтобы не пропустить первые 4 байта
				...
				if(GSM_RxCharN >= 4){
					...
					if(AppProtocol == 0){
						if(UART_Soft){
							...
							EM_InitFIFO();
							GSM_RX_FIFO_End_Transp = GSM_RX_FIFO_End_old;	// Чтобы не пропустить первые 4 байта
							Transparent = 1;
							StartTimer16(TD_TCP_Connect, erw(&TCP_CONNECT_timeout));	// запускаем таймер таймаута
						}
						break;
...
//+CLIP: "+380976969029",145,"",,"",0
		...
		case GSM_WAIT_CONNECT_9600:
			if(GSM_Wait_Response_P(RESP_CONNECT_9600, GSM_SEND_ATH)){
				StartTimer16(TD_TCP_Connect, 500);	// запускаем таймер таймаута
				GSM_CSD = 1;
				GSM_State = GSM_ProtocolMode;
			}
			break;
